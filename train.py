...
def load_data(file_path=TRAIN_FILE):
    ...
    df[LABEL_COLUMN] = df[LABEL_COLUMN].astype(str).str.strip().str.lower().replace({"mad": "bad", "negative": "bad", "positive": "good"})
...
def create_model():
    features = FeatureUnion([
        ("word_tfidf", TfidfVectorizer(
            analyzer="word", token_pattern=r"(?u)\b\w+\b", lowercase=False,
    ...
    return Pipeline([
        ("features", features),
        ("classifier", LogisticRegression(max_iter=2000, class_weight="balanced", solver="liblinear", random_state=RANDOM_STATE, C=1.0)),
    ])
